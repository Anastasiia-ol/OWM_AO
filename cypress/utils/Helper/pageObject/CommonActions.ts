...
export function checkElementIsSelected(locator, index = 0){
        cy.get(locator).eq(index).should('have.class', 'isSelected');
}
export function checkElementIsNotSelected(locator, index = 0){
        cy.get(locator).eq(index).then(($ell) => {
                expect($ell).not.to.be.checked
        })
}
export function checkElementIsChecked(locator, index = 0){
        cy.get(locator).eq(index).then(($ell) => {
